...
#### Structure of an XState Model Test:
* **The Abstract Machine Definition:** Defines allowable transitions (e.g., you cannot transition from `Pending` straight to `Revoked` without first going through `Authorized`).
* **The Meta Verification Block:** Every state contains a `meta.test` method that performs rigorous, cryptographic assertions on an in-memory mock system wrapper (e.g., verifying that the device *can* decrypt the AMK when in `Authorized`, and *fails* once transitioned to `Revoked`).
* **Path Generation:** The `@xstate/test` engine calculates the shortest and simplest mathematical paths to hit every state and edge transition, automatically generating and executing the test suites.
...
#### 1. Adversarial Byzantine & Chaos Commands
...
* **`MutateCiphertextCommand` (Ciphertext Tampering):** Injecting bit flips and truncating the beginning of base64 wrapped keys. The test verifies that tampering is caught during unwrap, throwing integrity verification exceptions.
...
* **Forward Secrecy Invariant:** Older rotated Master Keys are mathematically incapable of decrypting subsequent ledger events.
